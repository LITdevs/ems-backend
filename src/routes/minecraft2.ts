...
router.get("/status", Auth, (req: Request, res: Response) => {
    let minecraftStatusCommand = 'systemctl show fml | grep "StateChangeTimestamp\\|SubState\\|ActiveState"'
...
router.patch("/status", Auth, (req: Request, res: Response) => {
    ...
    if (desiredState) statusChangeCommand = "sudo systemctl start fml.service"; // Start server in screen with sockname atm
    if (!desiredState) statusChangeCommand = "sudo screen -S atm -p 0 -X stuff \"stop^M\""; // Sends stop command to the server console
    if (force) statusChangeCommand = "sudo systemctl stop fml.service"; // Force stop server
...
router.get("/icon", (req : Request, res: Response) => {
    res.sendFile("/litdevs/ems-internal/fml.png")
})
